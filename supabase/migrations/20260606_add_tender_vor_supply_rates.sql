-- task 398: расценки от снабжения по материалам ВОР тендера.
-- Привязка к тендеру + конкретному документу-ВОР (estimate_name).
-- Сопоставление с позициями ВОР — по наименованию материала.
-- Хранится отдельной таблицей, чтобы переживать переимпорт самого ВОРа
-- (DELETE/INSERT tender_estimate_items не трогает расценки).
CREATE TABLE IF NOT EXISTS tender_vor_supply_rates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tender_id UUID REFERENCES tenders(id) ON DELETE CASCADE,
    estimate_name TEXT NOT NULL,          -- имя ВОР-документа (как tender_estimate_items.estimate_name)
    material_name TEXT NOT NULL,          -- наименование материала
    unit TEXT,                            -- единица измерения
    supply_price DECIMAL(15, 2) NOT NULL, -- цена от снабжения за единицу
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    -- один материал = одна расценка в пределах документа тендера
...
-- RLS: пермиссивная политика для authenticated (паттерн проекта)
