...
// task 399: латинские буквы, визуально неотличимые от кириллических — частая
// причина «невидимого» несовпадения имён ВОР↔КП (ячейки набраны в разных
// раскладках). Сводим к кириллице. Только однозначные двойники, чтобы не портить
// латинские модели/маркировки (DN50, BFV-02/W) — они сворачиваются симметрично с
// обеих сторон, так что матчу это не мешает.
const LATIN_LOOKALIKES = { a: 'а', c: 'с', e: 'е', o: 'о', p: 'р', x: 'х', y: 'у' }

// Нормализация name/unit для match: lowercase, ё→е, надстрочные ²³→2/3,
// удаление невидимых символов (zero-width, мягкий перенос — JS \s их НЕ ловит),
// латинские двойники → кириллица, удаление кавычек/скобок, схлопывание пробелов и точек.
export function normalizeKey(s) {
  return String(s || '')
    ...
    .replace(/[acepxyo]/g, (ch) => LATIN_LOOKALIKES[ch] || ch)
    .replace(/[«»"'`()]/g, '')
    .replace(/[\s.]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
