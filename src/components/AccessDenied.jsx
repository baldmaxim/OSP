import { useNavigate } from 'react-router-dom'

// security fix: пользователь авторизован и роль загружена, но НЕТ права на раздел.
// Страница раздела при этом НЕ монтируется (никаких Supabase-запросов запрещённого раздела).
// title/message/backTo — необязательные: тем же компонентом показываем и отказ по объекту
// (руководитель привязан к своему объекту и открыл чужой по прямой ссылке).
export default function AccessDenied({
  title = 'Нет прав для просмотра раздела',
  message = 'У вас нет прав для просмотра этого раздела. Обратитесь к администратору, если доступ нужен.',
  backTo = '/general',
}) {
  const navigate = useNavigate()
  return (
    <div style={{
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      gap: '1rem', minHeight: '70vh', padding: '2rem', textAlign: 'center',
    }}>
      <div style={{ fontSize: '2.5rem' }} aria-hidden>⛔</div>
      <h1 style={{ fontSize: '1.25rem', margin: 0, color: 'var(--text-primary)' }}>{title}</h1>
      <p style={{ maxWidth: 460, color: 'var(--text-secondary)', margin: 0 }}>{message}</p>
      <button className="btn-primary" style={{ marginTop: '0.5rem' }} onClick={() => navigate(backTo)}>
        Вернуться на главную
      </button>
    </div>
  )
}
