  ...
  const sortedTenders = useMemo(() => {
    ...
    return [...filteredByTab].sort((a, b) => {
      ...
      if (sortField === 'status') {
        ...
        av = tenderNumberOf(a) ?? ''
        bv = tenderNumberOf(b) ?? ''
      } else if (sortField === 'parent_tender_start_date') {
        av = a.parent_tender?.tender_start_date || ''
        bv = b.parent_tender?.tender_start_date || ''
      } else {
        av = a[sortField] || ''
        bv = b[sortField] || ''
      }
        ...
        <div className="tp-header-actions" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          ...
          {isMaterialsView && (
            <button
              ...
              className="btn-view-toggle is-icon-only"
              title="Структура хранения документов"
              aria-label="Структура хранения документов"
            >
